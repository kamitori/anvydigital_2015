...
var Design = function() {
	...
	var svgSetup = {
		...
		texts:[
			...
			// 	x:50,
			// 	y:50,
			// 	text:"Double-click on the text to edit",
			// 	font:"Arial",
			// 	fontsize:15,
			// 	fontweight:500,
			// 	color: "red"
	...
	function drawSVG()
	{
		...
		for( var i in svgSetup.elements ) {
			...
			groupText = draw.group()
				.attr('id', 'group-text');
			for(var i in svgSetup.texts) {
				addText(svgSetup.texts[i], true);
			}
		}
	}
	...
	function setFont()
	{
		if( $('#svg-main #font-style').length ) {
			$('#svg-main #font-style').remove();
		}
		var html = '<style id="font-style" type="text/css">';
		for(var i in svgFonts) {
			html += '@font-face {' +
			    'font-family: "'+ svgFonts[i].name +'";' +
			    'src: url('+ svgFonts[i].source +');' +
			'}';
		}
		html += '</style>';
		$('#svg-main').append(html);
	}
	...
	function addText(options, inside)
	{
		if( typeof options  == 'undefined' ) {
			options = {};
		}
		options = $.extend({
						id: 'text-'+ svgSetup.texts.length,
						...
						stroke: '#000',
						path: [],
					}, __getCurrentPosition(event), options);
		var svgText = draw.text(options.text)
						.attr({
							id: options.id,
							x: options.x,
							y: options.y,
							...
							Main.changeText({
								'text': this.text(),
								'size': this.node.getAttribute('font-size'),
								'font': this.node.getAttribute('font-family'),
								'weight': this.node.getAttribute('font-weight'),
								'stroke-width': this.node.getAttribute('stroke-width'),
								'stroke': this.node.getAttribute('stroke'),
								'color': this.node.getAttribute('fill'),
								'textPath': this.textPath
							});
							Main.onOffText('on');
							Main.textPanel.find('textarea').focus();
							selectedText = this;
		...
		groupText.add(svgText.draggable());
		svgText.beforedrag = function() {
			if( $('#preview_box').is(':visible') ) {
				return false;
			}
			return true;
		};
		svgText.dragmove = function() {
			var id = this.node.getAttribute('id');
				id = id.replace('text-', '');
			svgSetup.texts[ id ].x = this.x();
			svgSetup.texts[ id ].y = this.y();
		};
		if( typeof inside == 'undefined' || !inside ) {
			svgText.fire('click');
			svgSetup.texts.push(options);
		}
	}
	...
	function addTextPath()
	{
		...
		var id = selectedText.id().replace('text-', '');
		$('#svg-main').find('image').each(function() {
			this.instance.addClass('freeze');
		});
		$('#svg-main').find('.main-image').css('cursor', 'crosshair');
		var textPath = false;
		$('#svg-main').mousedown(function(event) {
			textPath = true;
			svgSetup.texts[ id ].path = [];
		}).mousemove(function(event) {
			if( textPath ) {
				var current = __getCurrentPosition(event);
				svgSetup.texts[ id ].path.push(current.x +' '+current.y );
				textPath = svgSetup.texts[ id ].path;
				textPath = 'M'+ textPath.join('L');
				if( !$('#svg-main #temporary-text-path').length ) {
					draw.path(textPath).attr({
						'id': 'temporary-text-path',
						'stroke': '#000',
						'stroke-width': 2,
						'fill': 'none'
					});
				} else {
					$('#svg-main #temporary-text-path')[0].instance.plot(textPath);
				}
			}
		}).mouseup(function(event) {
			$('#svg-main').find('image').each(function() {
				this.instance.removeClass('freeze');
			});
			$(this).find('.main-image').css('cursor', 'move');
			$('#svg-main #temporary-text-path')[0].instance.remove();
			if( selectedText.textPath ) {
				selectedText.plot(textPath);
			} else {
				selectedText.path(textPath);
			}
			textPath = false;
	...
	function removeTextPath()
	{
		...
		var id = selectedText.id().replace('text-', '');
		svgSetup.texts[ id ].path = [];
		console.log(selectedText.textPath);
		selectedText.textPath.remove();
	}

	function __getCurrentPosition(event)
	{
		var svg   = $('#svg-main')[0];
		var pt    = svg.createSVGPoint();
		pt.x = event.clientX;
		pt.y = event.clientY;
		var current = pt.matrixTransform(svg.getScreenCTM().inverse());
		return {x: current.x, y: current.y - 10};
	}
	...
	return {
		...
		size: function(sizew, sizeh, reset) {
			...
			svgSetup.main.width = w + svg_bleed_pt;
			svgSetup.main.height = h + svg_bleed_pt;


			var bleed = svg_bleed_pt;
			var x = svg_bleed_pt;
			var y = svg_bleed_pt;
			var width = w;
			var height = h;
		...
		text: function() {
			$('#svg-main .main-image').css('cursor', 'text');
			$('#svg-main').one('click', function(){
				addText();
				$('#svg-main .main-image').css('cursor', '');
				Design.addText = true;
			});
		},
		changeText: function(type, value) {
			if( typeof selectedText == 'undefined' ) {
				return false;
			}
			var id = selectedText.id().replace('text-', '');
			switch(type) {
				case 'text':
					...
					svgSetup.texts[ id ].text = value;
					break;
				case 'color':
					selectedText.attr({'fill': value});
					svgSetup.texts[ id ]['color'] = value;
					break;
				case 'size':
					selectedText.font({size: value});
					svgSetup.texts[ id ].size = value;
					break;
				case 'weight':
					selectedText.font({weight: value});
					svgSetup.texts[ id ].weight = value;
					break;
				case 'font':
					selectedText.font({family: value});
					svgSetup.texts[ id ].family = value;
					break;
				case 'stroke':
					selectedText.attr({stroke: value});
					svgSetup.texts[ id ].stroke = value;
					break;
				case 'stroke-width':
					selectedText.attr({'stroke-width': value});
					svgSetup.texts[ id ]['stroke-width'] = value;
					break;
				case 'stroke':
					selectedText.attr({'stroke': value});
					svgSetup.texts[ id ]['stroke'] = value;
