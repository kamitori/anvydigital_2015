								 ...
								 <div class="col-md-10">
									<div class="row col-md-10">
									   <div class="form-group text-right">
											...
											<button id="add-image" class="btn green col-md-1" type="button" style="margin-right:10px;" data-toggle="popover" title="Image"><i class="fa fa-photo"></i></button>
											<button id="add-text" class="btn col-md-1" type="button" style="margin-right:10px;" data-toggle="popover" title="Text"><i class="fa fa-file-text-o"></i></button>
											...
											<div id="helpers" class="panel-collapse collapse" aria-expanded="false">
												<div class="panel-body">
													...
													<div class="col-md-4">
														...
														<span class="help-block">Use <kbd>page up</kbd> to send object to forward.</span>
														<span class="help-block">Use <kbd>page down</kbd> to send object to backward.</span>
													</div>
												</div>
											</div>
										</div>
...
var shapeLayout = function() {
	...
	{
		var x = parseFloat($('#w').val()) / 3;
		var y = parseFloat($('#y').val()) / 3;
		var w = x*2;
		var h = y*2;
		addShape(url, undefined, w, h, w, y, 'image', 0);
	}

	function addText(text)
	{
		var x = parseFloat($('#w').val()) / 2;
		var y = parseFloat($('#y').val()) / 2;
		addShape(text, undefined, undefined, undefined, x, y, 'text', 0);
	}

	function addShape(d, id, w, h, x, y, type, empty)
	{
		var shape;
		if( type == 'image' ) {
			var realX = x*config.dpi / config.view_dpi;
			var realY = y*config.dpi / config.view_dpi;

			var realW = w*config.dpi / config.view_dpi;
			var realH = h*config.dpi / config.view_dpi;
			shape = draw.image(d).attr({
				x: realX,
				y: realY,
				width: realW,
				height: realH
			...
			shape = draw.text(d).attr({
						x: realX,
						y: realY,
						text: d
					});
		} else {
			shape = draw.path(d).attr({
										'fill': config.color,
										'stroke'        : '#000000',
										'stroke-width'  : 1,
									});
		...
		shapeBind(
			shape,
			{ width: w, height: h, x: x, y: y, type: type, empty: empty }
		);
	}
		...
		shape.draggable(false)
				...
				.mousedown(function(event){
					if( event.shiftKey || event.ctrlKey ) {
						var type = shapes[ this.id() ].type;
						if( $.inArray(type, ['image', 'text']) !== -1 ) {
				...
				.dblclick(function(){
					...
					if( shapes[ id ].type == 'text' &&  shapes[ id ].popover == undefined ) {
						$(this.node).popover({
							content: '<ul class="list-inline"><li><textarea></textarea></li></ul>',
							html: true,
							placement: 'right',
							container: 'body',
							trigger: 'dblclick'
							...
							var object = this;
							var popoverId = $(object).attr('aria-describedby');
							$('#'+popoverId+' textarea').val( $(this).attr('text') )
														.focus()
														.keyup(function(){
															var text = $(this).val();
															object.instance.text(text);
															shapes[ $(object).attr('id') ].d = text;
															$(object).attr('text', text);
														});
						});
						shapes[ id ].popover = true;
					}
	...
	function updateShapeInput(shape, attribute)
	{
		...
		if( shape.type == 'image'  ) {
			shapes[id].d = shape.attr('href').replace('{{ URL }}/', '');
		} else if( shape.type == 'text' ) {
			shapes[id].d = shape.attr('text');
		} else {
			if( shape.type == 'path' ) {
	...
	function layoutBuilder()
	{
		...
		var x, y, w, h, id, d, empty;
			@foreach($layout['shapes'] as $shape)
				...
				@if( $shape['type'] == 'image' )
				d = '{{ URL.'/'.$shape['d'] }}';
				@elseif( $shape['type'] == 'text' )
				d = {{ json_encode(explode("\n", $shape['d'])) }};
				@else
				d = '{{ $shape['d'] }}';
				@endif
				empty = {{ $shape['empty'] or 0 }};
				addShape(d, id, w, h, x, y, type, empty);
			@endforeach
		@endif
	}
	...
	function eventBind()
	{
		...
		$('#add-text').click(function() {
			addText('DbClick to edit this text...');
		});
		...
		$('#add-image').popover({
		    content: '<ul class="list-inline"><li><button class="btn" type="button" onclick="shapeLayout.upload()">Upload</button></li><li><button class="btn" type="button" onclick="shapeLayout.chooseImage()">Choose</button></li></ul>',
		    html: true,
		    placement: 'right',
		    container: 'body',
		    trigger: 'click'
	...
	function formValidate()
	{
		layoutForm.validate({
			...
			submitHandler: function (form) {
				...
				layout.trigger("click");
				for(var i in shapes) {
					if( !$('#layout-builder-svg #'+ i).length ) {
						continue;
					}
					shapes[i].order_no = $('#layout-builder-svg #'+ i).index();
				}
				updateInputSVG();
	...
	return {
		...
		upload: function() {
			...
			$('#add-image').popover('hide');
			$('<input type="file" accept="image/*" id="shape-image-file" style="display: none" />').appendTo('body');
			$('#shape-image-file').trigger('click').change(function() {
		    	var data = new FormData();
		        data.append('image', $(this)[0].files[0]);
				$.ajax({
				    url: '{{ URL }}/admin/layouts/upload-image',
				    asycn: false,
				    type: 'POST',
				    data: data,
				    processData: false,
...
function chooseImage(object)
{
    ...
    shapeLayout.addImage(src);
    $(modal).modal("hide");
}
</script>
@extends('admin.layout.image-browser', ['controller' => 'layouts', 'holder' => '.tab-pane .active', 'itemWidth' => 125])
@stop
