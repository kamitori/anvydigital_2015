                    ...
                    <div class="form-group last">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                ...
                                <select id="tab_id" name="tab_id" class="form-control"></select>
                            </div>
                        </div>
                    </div>
                </form>
                    ...
                    <table class="table table-striped table-hover table-bordered" id="list-product-option-groups">
                        <thead>
                            ...
                            <tr role="row" class="filter">
                                ...
                                <td>
                                    <select class="form-control form-filter input-sm" name="search[tab_id]"></select>
                                </td>
...
var columnDefs = [
        ...
        },
        {
            ...
            "name"  : "description",
            "data" : function(row, type, val, meta) {
                return '<a href="javascript:void(0)" class="xeditable-input" data-escape="true" data-name="description" data-type="textarea" data-pk="'+row[1]+'" data-url="{{ URL.'/admin/product-option-groups/update-product-option-group' }}">'+row[3]+'</a>';
            }
        },
        {
            "targets": 4,
            "name"  : "option_group",
            "data" : function(row, type, val, meta) {
                return '<a href="javascript:void(0)" class="xeditable-select" data-escape="true" data-name="tab_id" data-type="select" data-pk="'+row[1]+'" data-value="'+row[4]+'" data-url="{{ URL.'/admin/product-option-groups/update-product-option-group' }}">'+row[5]+'</a>';
            }
        },
    ];
listRecord({
    ...
    fnDrawCallback: function(){
        ...
        $("a.xeditable-select","#list-product-option-groups").editable({
            source: tab
        });
    },
});
var optionGroupSelect = function() {
    var html = "";
    for(var i in tab) {
        html += '<option value="'+tab[i].value+'">'+tab[i].text+'</option>';
    }
    $("#tab_id", "#option-group-add-form").html(html);
    $("[name='search[tab_id]'").html(html);
}
optionGroupSelect();
...
$("#option-group-add-form").validate({
    ...
    submitHandler: function (form) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: $("input,textarea,select", "#option-group-add-form").serialize(),
