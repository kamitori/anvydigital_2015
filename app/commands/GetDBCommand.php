...
class GetDBCommand extends Command {
	...
	private function all()
	{
		...
		if( URL == 'http://vi-demo.anvyonline.com' ) {
			...
			foreach(['tb_contact', 'tb_company', 'tb_country', 'tb_province', 'tb_tax', 'tb_product', 'tb_settings'] as $collection) {
				...
				$command = 'mongodump -h '. JT_IP .' -u '.JT_USERNAME.' -p '.JT_PASS.' --port 27017 -d jobtraq -q "'.$query.'" -c '.$collection.' -o '.$path;
				exec($command);
		...
		$this->call('db:command', ['--convert-contacts' => true]);
		$this->call('db:command', ['--convert-employees' => true]);
		$arrData = [
					'event'		=> 'DBSync',
	...
	{
		return array(
			array('all', null, InputOption::VALUE_NONE, 'Sync all collection.', null),
		);
	}
}
