...
class MenusController extends AdminController {
	...
	public function menuReorder()
	{
		...
		if(!empty($arrPost)){
			...
			return Redirect::to(URL.'/admin/menus')->with(['flash_success'=> $updated ? 'Menu has been re-ordered.' : 'Nothing was changed.']);
		}
		if ( $ajax ) {
			return ['status'=> 'error', 'message' => 'We found nothing to re-order. Please check again.'];
		}
	...
	public function updateMenu()
	{
		...
		if(!empty($arrPost)){
			...
			$menu->column_no 	= (int)$arrPost['column_no'];
			...
			if( $permission ) {
	        	...
	        	if( $ajax ) {
					return ['status' => 'error', 'message' => implode('<br />', $pass->messages()->all() )];
		       	}
		...
		if( $ajax ) {
			return ['status' => 'error', 'message' => 'There is something wrong. Please refresh and try again.'];
		}
		return Redirect::to(URL.'/admin/menus');
	}
	...
	public function deleteMenu($id)
	{
		...
		if(!is_null($menu)){
			...
			if( Permission::can($this->layout->admin, "menus{$type}_delete_all") ) {
				...
				$arrReturn['message'] = 'You do not have permission to delete this menu.';
			}
		}
		return $arrReturn;
	}
