...
class ConfiguresController extends AdminController {
	...
	public function index()
	{
		...
		$configures = Configure::select('ckey', 'cvalue')
								->whereIn('ckey', ['title_site', 'meta_description', 'main_logo', 'favicon','key_word'])
								->get();
		$configure = [];
	...
	public function listConfigure()
	{
		...
		$configures = Configure::select('id', 'cname', 'ckey', 'cvalue', 'cdescription', 'active')
								->whereNotIn('ckey', ['title_site', 'meta_description', 'main_logo', 'favicon','key_word'])
			...
			if( $key == 'main_logo' ) {
				...
				if( $choose ) {
					$imageId = $choose;
   					...
   					$imageId = MyImage::upload(Input::file('main_logo'), public_path().DS.'assets'.DS.'images'.DS.'logos', 400, false);
				}
				if( $imageId ) {
					$path = MyImage::where('id', $imageId)->pluck('path');
				...
				if( !Input::hasFile('favicon') ) continue;
   				$imageId = MyImage::upload(Input::file('favicon'), public_path().DS.'assets'.DS.'favicons', 16, false, 'favicon');
   				$path = MyImage::where('id', $imageId)->pluck('path');
   				$configure = Configure::firstOrNew(['ckey'=> $key]);
				$configure->ckey = $key;
				$configure->cname = Str::title(str_replace('_', ' ', $key));
	...
	public function imageBrowser($page = 1)
    {
    	if( Request::ajax() ) {
			...
			return MyImage::imageBrowser('logos', $page, false);
		}
   		return App::abort(404);
    }
}