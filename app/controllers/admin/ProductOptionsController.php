...
class ProductOptionsController extends AdminController {
	...
	public function listProductOption()
   	{
		...
		$productOptions = ProductOption::select('options.id', 'options.name', 'options.description', 'option_group_id', 'option_groups.name as option_group_name')
												->with('images')
   	...
   	public function updateProductOption()
	{
		if( Input::has('pk') ) {
   		...
   		$option->option_group_id = (int)Input::get('option_group_id');
   		...
   		if( $pass ) {
			...
			if( Input::has('choose') ) {
				...
				$imageId = MyImage::upload(Input::file('image'), public_path().DS.'assets'.DS.'images'.DS.'product-options', 960);
			}

			if( $imageId ) {
	   			$option->images()->attach( $imageId );

				$arrReturn = ['status' => 'ok'];
    			$arrReturn['message'] = $option->name.'\'s image has been saved';
	   		}
   	...
   	public function imageBrowser($page = 1)
    {
    	if( Request::ajax() ) {
			...
			return MyImage::imageBrowser('product-options', $page, false);
		}
   		return App::abort(404);
    }
