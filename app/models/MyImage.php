...
class MyImage extends BaseModel {

	protected $table = 'images';

    public static function upload($file, $path, $width = 110, $makeThumb = true, $fileName = '', $returnPath = false)
    {
        if( !File::exists($path) ) {
    ...
    public static function imageBrowser($type = 'all', $page = 1, $getThumb = true)
    {
        ...
        if( $type == 'product-categories') {
            $categoryId = 0;
            if( is_array($getThumb) && isset($getThumb['categoryId']) ) {
                $categoryId = $getThumb['categoryId'];
            }
            if( $categoryId ) {
                //$images->whereRaw('id IN (SELECT image_id FROM imageables WHERE imageables.imageable_type = \'Product\' AND imageables.option like \'%"cover":1%\'  AND imageables.imageable_id IN ( SELECT product_id  FROM  products_categories  WHERE category_id = '. $categoryId .' )  )');
		$images->whereRaw('id IN (SELECT image_id FROM imageables WHERE imageables.imageable_type = \'Product\' AND imageables.imageable_id IN ( SELECT product_id  FROM  products_categories  WHERE category_id = '. $categoryId .' )  )');
            }
        } else if( $type != 'all' ) {
