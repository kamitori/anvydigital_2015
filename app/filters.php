...
App::error(function($exception){
	if( php_sapi_name() !== 'cli'
		...
		if( $code !== 404 ) {
			Notification::add($code, 'System||Error', '<span class="label label-sm label-danger">'.$code.'</span> '. $exception->getMessage().'.<br/>');
		}
		if( $isAdmin ) {
...
{
	if (Auth::user()->check()) return Redirect::to('/account');
});
